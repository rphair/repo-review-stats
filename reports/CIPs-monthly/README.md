...
* The `slurp*` files are also huge, but can also be included with each monthly update: since most content changes are appended material in chronological order: well suited for incremental updates by `git`. 
