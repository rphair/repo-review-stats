...
Therefore querying the API for reviews timestamped in a certain interval would miss them whenever the PR was submitted outside that interval, and the frequency of these "orphans" would increase significantly as the time interval becomes shorter.  The only way to obtain a deterministic set of reviews for any time interval (short or long) is to obtain the full historical data collection _and then_ filter it by date.
